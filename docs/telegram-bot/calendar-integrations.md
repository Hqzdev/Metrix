Telegram Bot src/integrations/calendar
...
CALENDAR_TOKEN_SECRET — длинный секрет для шифрования токенов
GOOGLE_CALENDAR_CLIENT_ID — идентификатор OAuth-клиента Google
GOOGLE_CALENDAR_CLIENT_SECRET — секрет OAuth-клиента Google
GOOGLE_CALENDAR_REDIRECT_URI — redirect URI для Google
MICROSOFT_CALENDAR_CLIENT_ID — идентификатор OAuth-клиента Microsoft
MICROSOFT_CALENDAR_CLIENT_SECRET — секрет OAuth-клиента Microsoft
MICROSOFT_CALENDAR_REDIRECT_URI — redirect URI для Microsoft

CALENDAR_TOKEN_SECRET используется для шифрования accessToken и refreshToken.
В локальной разработке есть fallback. В production должен быть задан отдельный длинный секрет.
...
calendar-connection-store.ts хранит подключения в data/calendar-connections.json.
...
google-calendar-adapter.ts работает с OAuth endpoint Google, Calendar events API и FreeBusy API.

Создание события выполняется через Calendar events API.
Удаление события выполняется через Calendar events API.
Проверка занятости выполняется через FreeBusy API.

Microsoft Outlook

microsoft-calendar-adapter.ts работает с Microsoft Graph.

Создание события выполняется через Microsoft Graph events.
Удаление события выполняется через Microsoft Graph events.
Проверка занятости выполняется через Microsoft Graph getSchedule.
...
После этого listAvailableSlots(resourceId) скрывает уже забронированные слоты и слоты, занятые во внешнем календаре.
...
по умолчанию выполняется 3 попытки
между попытками используется небольшая задержка
